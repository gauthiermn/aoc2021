...
fs.readFile(filename, 'utf8', function (err, data) {
    ...
    lines.forEach(ln => {
        ...
        if ((ln.pt1.y != ln.pt2.y) && (ln.pt1.x != ln.pt2.x)) {
            var xdir = (ln.pt2.x - ln.pt1.x) / Math.abs(ln.pt2.x - ln.pt1.x);
            var ydir = (ln.pt2.y - ln.pt1.y) / Math.abs(ln.pt2.y - ln.pt1.y);
            for (let d = 0; d < Math.abs(ln.pt2.x - ln.pt1.x) + 1; d++) {
                grid[ln.pt1.x + (d * xdir)][ln.pt1.y + (d * ydir)]++;
            }
        }
    })
    var sum = 0;
